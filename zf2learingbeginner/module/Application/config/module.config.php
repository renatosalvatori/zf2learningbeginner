<?php
/**
 * Zend Framework (http://framework.zend.com/)
 *
 * @link      http://github.com/zendframework/ZendSkeletonApplication for the canonical source repository
 * @copyright Copyright (c) 2005-2013 Zend Technologies USA Inc. (http://www.zend.com)
 * @license   http://framework.zend.com/license/new-bsd New BSD License
 */

return array(
    'router' => array(
        'routes' => array(
            'home' => array(
                'type' => 'Zend\Mvc\Router\Http\Literal',
                'options' => array(
                    'route'    => '/',
                    'defaults' => array(
                        'controller' => 'Application\Controller\Index',
                        'action'     => 'index',
                    ),
                ),
            ),
            // The following is a route to simplify getting started creating
            // new controllers and actions without needing to create a new
            // module. Simply drop new controllers in, and you can access them
            // using the path /application/:controller/:action
            'application' => array(
                'type'    => 'Literal',
                'options' => array(
                    'route'    => '/application',
                    'defaults' => array(
                        '__NAMESPACE__' => 'Application\Controller',
                        'controller'    => 'Index',
                        'action'        => 'index',
                    ),
                ),
                /*'may_terminate' => true,
                'child_routes' => array(
                    'default' => array(
                        'type'    => 'Segment',
                        'options' => array(
                            'route'    => '/[:controller[/:action]]',
                            'constraints' => array(
                                'controller' => '[a-zA-Z][a-zA-Z0-9_-]*',
                                'action'     => '[a-zA-Z][a-zA-Z0-9_-]*',
                            ),
                            'defaults' => array(
                            ),
                        ),
                    ),
                ),*/
            ),
            'blog' => array(
            		'type'    => 'Literal',
            		'options' => array(
            				'route'    => '/blog',
            				'defaults' => array(
            						'__NAMESPACE__' => 'Application\Controller',
            						'controller'    => 'Blog',
            						'action'        => 'listview',
            						'pagecache'		=> false
            				),
            		),
            		'may_terminate' => true,
            		 'child_routes' => array(
            		 		'paginator-blog' => array(
            		 					
        								'type'    => 'Segment',
        								'options' => array(      										
        										'route'    => '/[:controller[/:action[/page/:page]]]',
        										'constraints' => array(
        												'page' => '[0-9]*',
        										),
        										'defaults' => array(
        												'__NAMESPACE__' => 'Application\Controller',
        												'controller'    => 'Blog',
        												'action'        => 'listview'
        										),
        								),
        					),
        					'manage' => array(
        					
        							'type'    => 'Segment',
        							'options' => array(
        									'route'    => '[/:controller[/:action[/:id]]]',
        									'constraints' => array(
        											'page' => '[0-9]*',
        									),
        									'defaults' => array(
        											'__NAMESPACE__' => 'Application\Controller',
        											'controller'    => 'Blog',
        											'action'        => 'manage'
        									),
        							),
        					),
        					'delete' => array(
        							'type'    => 'Segment',
        							'options' => array(
        									'route'    => '[/:controller[/:action[/:id]]]',
        									'constraints' => array(
        											'page' => '[0-9]*',
        									),
        									'defaults' => array(
        											'__NAMESPACE__' => 'Application\Controller',
        											'controller'    => 'Blog',
        											'action'        => 'delete'
        									),
        							),
        					),
            		 ),
            ),
            'access-denie'=>array(
                'type'    => 'Literal',
                'options' => array(
                    'route'    => '/accessdenie',
                    'defaults' => array(
                        '__NAMESPACE__' => 'Application\Controller',
                        'controller'    => 'Index',
                        'action'        => 'notallowed',
                    ),
                ),
            ),
        ),
    ),
    'service_manager' => array(
        'factories' => array(
            'translator' => 'Zend\I18n\Translator\TranslatorServiceFactory',
            'servicelogger'=>'Application\Service\Factory\LoggerService',
            'sessionmanager'=>'Application\Service\Factory\SessionService',
            'text-cache' => 'Zend\Cache\Service\StorageCacheFactory',
            'var-cache'	 => 'Application\Service\Factory\VariableCache',
        ),
        'invokables'=>array(
        	'table-gateway'=> 'Application\Service\Invokable\TableGateway',
        ),
        'services'=>array(
		        'logger_config'=>array(
		        		'path_filename'=>dirname(__FILE__),
		        		'log_filename'=>'LOGGER_'.date('ymd',time()),
		        		'priority'=>'7'
		        ),
		        'session_config_application'=>array(),
        )
    ),
    'translator' => array(
        'locale' => 'en_US',
        'translation_file_patterns' => array(
            array(
                'type'     => 'gettext',
                'base_dir' => __DIR__ . '/../language',
                'pattern'  => '%s.mo',
            ),
        ),
    ),
    'controllers' => array(
        'invokables' => array(
            'Application\Controller\Index' => 'Application\Controller\IndexController',
            'Application\Controller\Blog' => 'Application\Controller\BlogController'
        ),
    ),
    'view_manager' => array(
        'display_not_found_reason' => true,
        'display_exceptions'       => true,
        'doctype'                  => 'HTML5',
        'not_found_template'       => 'error/404',
        'exception_template'       => 'error/index',
        'template_map' => array(
            'layout/layout'           => __DIR__ . '/../view/layout/layout.phtml',
            'application/index/index' => __DIR__ . '/../view/application/index/index.phtml',
            'error/404'               => __DIR__ . '/../view/error/404.phtml',
            'error/index'             => __DIR__ . '/../view/error/index.phtml',
        ),
        'template_path_stack' => array(
            __DIR__ . '/../view',
        ),
    ),
    'table-gateway' => array(
    			'map' => array(
    				'category'=>'Application\Model\Category',
    				'product'=>'Application\Model\Product',
    				'blog'=>'Application\Model\Blog'
    			)
    ),
    'acl' => array(
    		/**
    		 * Access Control List
    		 * -------------------
    		*/
    
    		'resources' => array(
    				'allow' => array(
    						'Application\Controller\Index' => array(
										'index'	=> 'guest',
										'notallowed'	=> 'guest', 
						),
							'Application\Controller\Blog' => array(
								'index'	=> 'guest',
								'listview'	=> 'guest',
								'manage'	=> 'guest',
								'delete'=>'member'
						),
						'blog_usergrid'=>array(
							'edit'=>'member',
							'delete'=>'member'
						)
    				),
    				'deny' => array(
    							
    				)
    		)
    ),
    'navigation' => array(
    		'default' => array(
    				array(
    						'label' => 'Home',
    						'route' => 'home',
    						'resource' => 'Application\Controller\Index',
    						'privilege' => 'index',
    				),
    		)
    )
);
